package com.ddl.sdutnews.utils;
...
public class HtmlUtil {
    ...
    public static void init(int flag,int page) throws IOException {        //初始化，加上flag方便并行
        list.set(flag-1,get_list_form_html(flag,page));
    }
    public static List<ListEleBean> get_list(int flag) {            //接口获得新闻列表
        return list.get(flag-1);
    }
    private static List<ListEleBean> get_list_form_html(int flag,int page) throws IOException {    //从html中解析新闻列表
        List<ListEleBean> list = new ArrayList<ListEleBean>();
        String html_pre = new String();
        String html = new String();
        html_pre = h.get(flag-1);
        html = html_pre + page + ".htm";
